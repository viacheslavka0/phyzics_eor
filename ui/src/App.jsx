...
function LearningView() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Mobile top bar */}
      <div className="sidebar-mobile-toggle fixed top-0 left-0 right-0 z-[200] bg-white/95 backdrop-blur-sm border-b border-slate-200/60 text-slate-900 flex items-center gap-3 px-4 py-3">
        <button onClick={() => setSidebarOpen(true)} className="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center text-lg text-slate-700" aria-label="Меню">☰</button>
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-sm truncate text-slate-900">{ksData.title}</div>
          <div className="text-[11px] text-slate-500 truncate">{displayName} · {roleLabel}</div>
        </div>
        {forcedStudentView && viewerUser?.is_staff && onReturnToTeacher && (
          <button
            type="button"
            onClick={onReturnToTeacher}
            className="px-2.5 py-1.5 rounded-lg bg-indigo-600 text-white text-[11px] font-medium"
          >
            Панель учителя
          </button>
        )}
      ...
      >
        ...
        <div className={`p-3 border-b border-slate-200 flex items-center ${sidebarCollapsed ? "justify-center" : "justify-between"}`}>
          ...
          <button
            ...
            className={`w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-700 transition-colors text-sm ${sidebarCollapsed ? "hidden group-hover:flex" : ""}`}
        ...
        <div className={`flex flex-col flex-1 overflow-hidden ${sidebarCollapsed ? "hidden group-hover:flex" : "flex"}`}>
            ...
            <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              ...
              {forcedStudentView && viewerUser?.is_staff && onReturnToTeacher && (
                <button
                  type="button"
                  onClick={onReturnToTeacher}
                  className="mt-2 w-full px-2 py-1.5 rounded-md border border-indigo-400/40 text-indigo-600 text-xs hover:bg-indigo-50 transition-colors"
                >
          ...
          <div className="p-5 border-t border-slate-200 bg-slate-50/80">
            <div className="grid grid-cols-2 gap-4 text-center mb-4">
              ...
              <div>
                <div className="text-2xl font-bold text-slate-600">{session?.tasks_solved_count || 0}</div>
            ...
            {isPilotMode && (
              <button
                ...
                disabled={resetting}
                className="w-full px-3 py-2 text-xs text-red-500 border border-red-300/60 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-50"
              >
            ...
            <button
              type="button"
              onClick={handleLogout}
              className="w-full mt-2 px-3 py-2 text-xs text-slate-600 border border-slate-300/60 rounded-lg hover:bg-slate-100 transition-colors"
            >
              Выйти из аккаунта
            </button>
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-2">
      ...
      <div className="card p-5 md:p-7">
        ...
        {/* Плашка цели вверху — тонкая */}
        <div className="flex items-center gap-2 mb-5 text-xs text-indigo-700 bg-indigo-50 rounded-lg px-3 py-2 border border-indigo-100">
          <span className="text-base">🎯</span>
          <span>{DEFAULT_TASK_FORMULATION}</span>
        </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <h3 className="text-xl font-bold tracking-tight text-slate-900">
                {currentTask.title || `Ситуация ${taskNumber}`}
              </h3>
            </div>

            {/* Task text */}
            <div
              id="task-problem-card"
              className="rounded-xl bg-slate-50 border border-slate-200 p-4 md:p-5"
            >
              <p className="text-slate-800 leading-relaxed" style={{ fontSize: "1.1rem" }}>
                {currentTask.text}
              </p>
            </div>
          </div>

          {/* Иллюстрация — квадрат справа, только если есть */}
          {currentTask.illustration_url && (
            <div className="shrink-0 hidden sm:block">
              <img
                src={currentTask.illustration_url}
                alt="Иллюстрация к задаче"
                className="w-44 h-44 md:w-52 md:h-52 rounded-2xl object-cover shadow-sm border border-slate-100"
              />
            </div>
          )}
        </div>

        {/* Иллюстрация на мобильном — под текстом, полная ширина */}
        {currentTask.illustration_url && (
          <div className="sm:hidden mb-5">
            <img
              src={currentTask.illustration_url}
              alt="Иллюстрация к задаче"
              className="w-full max-h-52 rounded-xl object-cover border border-slate-100"
            />
          </div>
        )}
        ...
        <div className="space-y-3">
            ...
            <div id="task-answer-card" className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <label className="block text-sm font-semibold text-slate-700 mb-2.5">
                Ваш ответ
                {!result && (
                  <span className="ml-2 font-normal text-slate-400 text-xs">
                    {photoRequired ? "· решите в тетради, введите число и фото" : "· решите в тетради, введите число"}
                  </span>
                )}
              </label>
            ...
            {/* Фото решения — компактнее */}
            <div id="task-photo-card" className="rounded-xl border border-slate-200 bg-white p-4">
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                Фото решения из тетради{" "}
                {photoRequired ? (
                  <span className="text-rose-500 font-medium">· обязательно</span>
                ) : (
                  <span className="text-slate-400 font-normal">· по желанию</span>
                )}
              </label>
              <label
              ...
              >
                ...
                <span className="text-sm text-slate-500 group-hover:text-indigo-700 transition-colors">
                  {answerPhotos.length ? `${answerPhotos.length} фото добавлено` : "Нажмите или перетащите фото"}
                </span>
              </label>
              <input
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-2">
        ...
        {/* Step navigation + progress — единый блок */}
        <div className="card px-5 pt-3 pb-4 mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-emerald-600 font-semibold whitespace-nowrap">
              {activeSteps.filter(s => stepAttempts[s.order]?.final_answer).length} / {activeSteps.length} выполнено
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-5">
            <div className="lg:sticky lg:top-4 space-y-4">
              <div className="card p-5">
                ...
                <div className="mb-3">
                  <h3 className="text-base font-bold text-slate-700 uppercase tracking-wider text-xs mb-1">Условие задачи</h3>
                </div>
          ...
          <div className="mb-5 pb-4 border-b border-slate-100">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-semibold text-indigo-500 uppercase tracking-wider">Шаг {currentStepIndex + 1}</span>
            </div>
            ...
            {currentStep.step_type === "text_pick" && (
              <>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Нажмите на нужные слова в тексте:
                </label>
                <div className="rounded-xl border border-indigo-200 bg-indigo-50/30 p-4">
                  <div className="leading-9 text-slate-800">
                    {selectableTokens.map((token) => {
                      ...
                      return (
                        <button
                          ...
                          className={`mx-[2px] inline rounded px-1 py-0.5 transition-all cursor-pointer ${
                            isSelected
                  ...
                  {studentAnswers[currentStepOrder] && (
                    <div className="mt-3 pt-2.5 border-t border-indigo-100 flex items-center gap-2">
                      <span className="text-xs text-slate-500">Выбрано:</span>
                      <span className="text-sm font-semibold text-indigo-800 bg-indigo-100 px-2 py-0.5 rounded-lg">
                        {studentAnswers[currentStepOrder]}
                      </span>
                    </div>
                  )}
          ...
          {attempt && !attempt.final_answer && (
            <div ref={stepResultRef} className="rounded-2xl border border-slate-200 bg-white shadow-sm mb-5 overflow-hidden">
              ...
              <div className="px-5 py-4 bg-slate-50 border-t border-slate-200">
                <p className="text-sm font-semibold text-slate-700 mb-3">Совпадает ли ваш ответ с эталоном?</p>
                <div className="flex gap-3">
                  <button
                    onClick={() => handleChooseVariant(currentStepOrder, false)}
                    className="flex-1 px-4 py-2.5 rounded-xl border-2 border-indigo-200 text-indigo-700 font-semibold text-sm hover:bg-indigo-50 transition-colors"
                  >
                    ✓ Да, совпадает
                  </button>
                  <button
                    onClick={() => handleChooseVariant(currentStepOrder, true)}
                    className="flex-1 px-4 py-2.5 rounded-xl bg-slate-700 text-white font-semibold text-sm hover:bg-slate-800 transition-colors"
                  >
                    Нет — взять эталон
                  </button>
                </div>
    ...
    <div className="max-w-3xl mx-auto">
      <div className="card p-6 md:p-8 animate-fadeIn">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-3 max-w-xl mx-auto leading-relaxed animate-slideUp" style={{ animationDelay: "0.3s" }}>
