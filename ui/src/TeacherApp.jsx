...
export default function TeacherApp({ currentUser = null, onSwitchToStudentView = null }) {
  const [view, setView] = useState("pilot-dashboard"); // pilot-dashboard | pilot-organizer | structure | ks-edit | tasks | elements | final-reviews | gradebook | research
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {view === "research" && (
          <Suspense fallback={<div className="text-slate-400 text-sm py-12 text-center">Загрузка модуля…</div>}>
            <ResearchDashboard />
          </Suspense>
        )}
      </main>
...
// Ручки ресайза: 8 позиций
const HANDLES = [
  { id: "nw", cursor: "nw-resize", top: -5,  left: -5,  style: {} },
  { id: "n",  cursor: "n-resize",  top: -5,  left: "50%", style: { transform: "translateX(-50%)" } },
  { id: "ne", cursor: "ne-resize", top: -5,  right: -5, style: {} },
  { id: "w",  cursor: "w-resize",  top: "50%", left: -5, style: { transform: "translateY(-50%)" } },
  { id: "e",  cursor: "e-resize",  top: "50%", right: -5, style: { transform: "translateY(-50%)" } },
  { id: "sw", cursor: "sw-resize", bottom: -5, left: -5, style: {} },
  { id: "s",  cursor: "s-resize",  bottom: -5, left: "50%", style: { transform: "translateX(-50%)" } },
  { id: "se", cursor: "se-resize", bottom: -5, right: -5, style: {} },
];
...
function ZoneEditor({ imageUrl, zones, onZonesChange }) {
  ...
  // Рефы для drag/resize — не вызывают ре-рендер на каждый mousemove
  const dragRef   = React.useRef(null); // { zoneId, offsetX, offsetY }
  const resizeRef = React.useRef(null); // { zoneId, handle, startX, startY, origZone }
  ...
  const handleMouseDown = (e) => {
    ...
    if (e.target.dataset.handle) {
      ...
      resizeRef.current = { zoneId, handle: e.target.dataset.handle, startX: pos.x, startY: pos.y, origZone: { ...zone } };
      return;
    }

    // — Перемещение существующей зоны
    ...
    if (zoneEl && zoneEl.dataset.zoneId) {
      ...
      dragRef.current = { zoneId, offsetX: pos.x - zone.x, offsetY: pos.y - zone.y };
      return;
    }

    // — Рисование новой зоны (только по IMG)
  ...
  const handleMouseMove = (e) => {
    ...
    if (resizeRef.current) {
      const { zoneId, handle, startX, startY, origZone } = resizeRef.current;
      const dx = pos.x - startX;
      const dy = pos.y - startY;
      let { x, y, width, height } = origZone;
      if (handle.includes("e")) { width  = Math.max(20, origZone.width  + dx); }
      if (handle.includes("s")) { height = Math.max(20, origZone.height + dy); }
      if (handle.includes("w")) { x = origZone.x + dx; width  = Math.max(20, origZone.width  - dx); }
      if (handle.includes("n")) { y = origZone.y + dy; height = Math.max(20, origZone.height - dy); }
      onZonesChange(zones.map(z => z.id === zoneId ? { ...z, x: Math.round(x), y: Math.round(y), width: Math.round(width), height: Math.round(height) } : z));
      return;
    }

    // Перемещение
    if (dragRef.current) {
      const { zoneId, offsetX, offsetY } = dragRef.current;
      onZonesChange(zones.map(z => z.id === zoneId
        ? { ...z, x: Math.round(pos.x - offsetX), y: Math.round(pos.y - offsetY) }
        : z
      ));
      return;
    }
    ...
    setCurrentRect({
      ...
      width:  Math.round(Math.abs(pos.x - startPos.x)),
      height: Math.round(Math.abs(pos.y - startPos.y)),
    });
  };
  ...
  const handleMouseUp = () => {
    ...
    if (isDrawing && currentRect && currentRect.width > 20 && currentRect.height > 20) {
      const newZone = { id: `zone_${Date.now()}`, ...currentRect, label: `Зона ${zones.length + 1}` };
  ...
  const handleUpdateZone = (zoneId, field, value) => {
    ...
    onZonesChange(zones.map(z => z.id === zoneId
      ? { ...z, [field]: isNaN(num) ? z[field] : Math.max(0, num) }
      : z
    ));
  };
  ...
  return (
    <div className="space-y-4">
      {/* Подсказка */}
      <p className="text-sm text-slate-500">
        Тяните на изображении, чтобы нарисовать новую зону. Щёлкните по зоне, чтобы выбрать и отредактировать.
      </p>
      ...
      <div className="flex gap-4 items-start flex-wrap">
        ...
        <div
          ref={containerRef}
          className="relative inline-block border border-slate-300 rounded-lg overflow-hidden select-none flex-shrink-0"
            ...
            return (
              <div
                ...
                className={`absolute border-2 group ${
                  isSelected
                    ? "border-indigo-500 bg-indigo-500/15 cursor-move"
                    : "border-yellow-500 bg-yellow-500/15 cursor-pointer hover:border-yellow-400"
                }`}
                style={{ left: zone.x, top: zone.y, width: zone.width, height: zone.height }}
              >
                {/* Метка */}
                <span
                  className={`absolute -top-5 left-0 text-xs text-white px-1.5 py-0.5 rounded whitespace-nowrap pointer-events-none ${
                    isSelected ? "bg-indigo-500" : "bg-yellow-500"
                  }`}
                >
                  {zone.label}
                </span>

                {/* Ручки ресайза — только для выбранной зоны */}
        ...
        {/* Панель редактирования выбранной зоны */}
        {selected && (
          <div className="flex-1 min-w-48 bg-indigo-50 border border-indigo-200 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-indigo-800">Редактировать зону</h4>
              <button
                onClick={() => handleDeleteZone(selected.id)}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => onZonesChange(zones.map(z => z.id === selected.id ? { ...z, label: e.target.value } : z))}
                className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { field: "x",      label: "X (лево)" },
                { field: "y",      label: "Y (верх)" },
                { field: "width",  label: "Ширина" },
                { field: "height", label: "Высота" },
              ].map(({ field, label }) => (
                <div key={field}>
                  ...
                  <input
                    ...
                    onChange={(e) => handleUpdateZone(selected.id, field, e.target.value)}
                    className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300"
                  />
                </div>
              ))}
            ...
            <p className="text-xs text-slate-400">
              Или перетащите / потяните за ручки прямо на изображении.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex flex-wrap gap-2">
            {zones.map((zone) => (
              ...
              >
                {zone.label}
                <span className="ml-1.5 text-xs opacity-50">{zone.width}×{zone.height}</span>
              </button>
            ))}
          </div>
        </div>
...
function TaskIllustrationUploader({ taskId, currentUrl }) {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      fd.append("illustration", file);
      const res = await fetch(`/api/teacher/tasks/${taskId}/upload_illustration/`, {
        method: "POST",
        credentials: "include",
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/teacher/tasks/${taskId}/delete_illustration/`, {
        method: "DELETE",
        credentials: "include",
        headers: { "X-CSRFToken": getCSRFCookie() },
      });
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm mt-4">
      <h3 className="text-base font-semibold text-slate-800 mb-4">🖼 Иллюстрация к задаче</h3>
      <div className="flex items-start gap-6">
        {/* Preview */}
        <div
          className="w-36 h-36 rounded-xl border-2 border-dashed border-slate-300 flex items-center justify-center bg-slate-50 overflow-hidden shrink-0 cursor-pointer hover:border-indigo-400 transition-colors"
          onClick={() => inputRef.current?.click()}
        >
          {url ? (
            <img src={url} alt="Иллюстрация" className="w-full h-full object-cover" />
          ) : (
            <span className="text-slate-400 text-xs text-center px-2">Нажмите, чтобы загрузить</span>
          )}
        </div>
        <div className="space-y-2">
          <p className="text-sm text-slate-600">
            Квадратная картинка, помогающая ученику представить сценарий задачи.<br />
...
function TaskEditor({ task, ksList, onBack }) {
  ...
  return (
    <div>
      ...
      {!isNew && task && (
        <TaskIllustrationUploader
          taskId={task.id}
          currentUrl={task.illustration_url || null}
        />
      )}
